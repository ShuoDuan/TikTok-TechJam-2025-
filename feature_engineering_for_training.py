...
def rate_authenticity_score_vectorized(df):
    ...
    # Pre-calculate restaurant averages
    restaurant_avg_ratings = df.groupby('business_name')['rating'].mean().to_dict()
    restaurant_counts = df.groupby('business_name').size().to_dict()
...
def process_all_features(filename='real_reviews_labeled.csv'):
    try:
        # Load data
...
if __name__ == "__main__":
    ...
    df = process_all_features('real_reviews_labeled.csv')

    if df is not None:
        # Save with both raw data and features
        df.to_csv('enhanced_reviews_with_features.csv', index=False)
